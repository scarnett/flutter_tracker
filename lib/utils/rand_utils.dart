import 'dart:math' as math;

const ASCII_START = 33;
const ASCII_END = 126;
const NUMERIC_START = 48;
const NUMERIC_END = 57;
const LOWER_ALPHA_START = 97;
const LOWER_ALPHA_END = 122;
const UPPER_ALPHA_START = 65;
const UPPER_ALPHA_END = 90;

// Generates a random integer where [from] <= [to].
int randomBetween(
  int from,
  int to,
) {
  if (from > to) {
    throw Exception('$from cannot be > $to');
  }

  math.Random rand = math.Random();
  return ((to - from) * rand.nextDouble()).toInt() + from;
}

// Generates a random string of [length] with characters
// between ascii [from] to [to].
// Defaults to characters of ascii '!' to '~'.
String randomString(
  int length, {
  int from: ASCII_START,
  int to: ASCII_END,
}) {
  return String.fromCharCodes(
      List.generate(length, (index) => randomBetween(from, to)));
}

// Generates a random string of [length] with only numeric characters.
String randomNumeric(
  int length,
) =>
    randomString(
      length,
      from: NUMERIC_START,
      to: NUMERIC_END,
    );

// Generates a random string of [length] with only alpha characters.
String randomAlpha(
  int length,
) {
  int lowerAlphaLength = randomBetween(0, length);
  int upperAlphaLength = (length - lowerAlphaLength);
  String lowerAlpha = randomString(
    lowerAlphaLength,
    from: LOWER_ALPHA_START,
    to: LOWER_ALPHA_END,
  );

  String upperAlpha = randomString(
    upperAlphaLength,
    from: UPPER_ALPHA_START,
    to: UPPER_ALPHA_END,
  );

  return randomMerge(lowerAlpha, upperAlpha);
}

// Generates a random string of [length] with alpha-numeric characters.
String randomAlphaNumeric(
  int length,
) {
  int alphaLength = randomBetween(0, length);
  int numericLength = (length - alphaLength);
  String alpha = randomAlpha(alphaLength);
  String numeric = randomNumeric(numericLength);
  return randomMerge(alpha, numeric);
}

// Merge [a] with [b] and scramble characters.
String randomMerge(
  String a,
  String b,
) {
  List<int> mergedCodeUnits = List.from('$a$b'.codeUnits);
  mergedCodeUnits.shuffle();
  return String.fromCharCodes(mergedCodeUnits);
}